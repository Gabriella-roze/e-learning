<?php
  ...
  // Get quizz data
  try {
    $q = $db->prepare('SELECT * FROM exam.quiz(3,5)');
    $q->execute();
    $quiz = $q->fetchAll();
  ...
  // Get topic from db
  try {
    $q = $db->prepare('SELECT * FROM exam.topic_view WHERE topic_id = 5');
    $q->execute();
    $topic = $q->fetchAll()[0];  
  } catch(PDOException $ex) {
  echo $ex->getMessage();
  ...
  try {
    $q = $db->prepare('SELECT * FROM exam.resources_view WHERE topic_id = 5 ORDER BY creation_date::DATE DESC');
    $q->execute();
